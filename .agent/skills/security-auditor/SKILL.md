...
1. **Zero-Trust Lockdown**: Verify all Firebase Security Rules using the `auth-security-architect` and `auth_data_auditor` workflows.
2. **Secret Scrying**: Mandate `dv scan-secrets` before any commit. Never allow raw API keys to survive.
3. **Deep Purgatory**: Delegate deep penetration testing and threat modeling to the `007` skill.
4. **Credential Sanctity**: Store all discovered secrets in Google Cloud Secret Manager immediately.
